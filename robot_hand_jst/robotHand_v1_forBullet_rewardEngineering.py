...
'''logの保存ファイル'''
save_file = 'test_rew=dizital_state=position.csv'
...
class Brain:
    ...
    def digitize_state(self, observation):
        ...
        digitized = [
            np.digitize(targetPosInHand_x, bins=self.bins(-1.0,
                                                          1.0, NUM_DIZITIZED)),  # 　面積の比率
            np.digitize(targetPosInHand_y,
                        bins=self.bins(-1.0, 1.0, NUM_DIZITIZED))
        ]
        # 　6進数で表して計算を圧縮
    ...
    def update_Q_table(self, observation, action, reward, observation_next):
        ...
        self.q_table[state, action] = self.q_table[state, action] + lr * \
            (reward + discount * Max_Q_next - self.q_table[state, action])
...
class Environment:
    ...
    def reset(self):
        ...
        p.setAdditionalSearchPath(
            os.environ['HOME']+"/atsushi/catkin_ws/src/robotHand_v1/urdf/")
        startPos = [0, 0, 0.03]
        ...
        # ターゲットを表示
        for i in range(1):
            targetX, targetY = np.random.permutation(
                np.arange(-1, 1, 0.1))[0:2]  # 2x2マス以内にランダムで配置
            targetPos = [targetX, targetY, 0.05]
            self.target = p.loadURDF(
                "target.urdf", targetPos, startOrientation)
    ...
    def get_env(self):
        ...
        targetPosInHand, targetOrnInHand = p.multiplyTransforms(
            invHandPos, invHandOrn, targetpos, targetorn)
        # 観測
        # ハンドからみたターゲットの相対位置(x,y)
        observation = (targetPosInHand[0], targetPosInHand[1])
        ...
        for episode in range(NUM_EPISODES):  # 試行数分繰り返す
            ...
            if (episode % test_interval == 0 and episode != 0) or TEST_MODE:
                ...
                for step in range(MAX_STEPS):  # 1エピソードのループ
                    if RENDER:
                        print('Step: {0} of Episode: {1}'.format(
                            step+1, episode))
                    # 行動を求める
                    action = self.agent.get_action(
                        observation, episode, test=False)
                    # 行動a_tの実行により、s_{t+1}, r_{t+1}を求める
                    observation_next, done = self.act_env(observation, action)

                    # 報酬を与える
                    # reward = observation_next[0]  # 面積値
                    # reward = self.reward(self.hand, self.target)  # 距離
                    reward = self.reward(step, done)  # 離散値
    ...
    def test(self, num_episodes):
        ...
        for episode in range(num_episodes):  # 試行数分繰り返す
            ...
            for step in range(MAX_STEPS):  # 1エピソードのループ
                ...
                if done:
                    print('{0} Episode: Finished after {1} time steps'.format(
                        episode, step+1))
                    images[0].save('test_success_episode.gif', save_all=True,
                                   append_images=images[1:], optimize=False, duration=1000)  # 成功エピソードの映像を保存
                    break
        # testの報酬のログをcsvで保存
        ...
        # 最終エピソードの動き
        images[0].save('test_final_episode-1.gif', save_all=True,
                       append_images=images[1:], optimize=False, duration=1000)
