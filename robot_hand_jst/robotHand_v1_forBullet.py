...
class Brain:
    ...
    def digitize_state(self, observation):
        ...
        digitized = [
            np.digitize(area_sum, bins=self.bins(
                0, 100.0, NUM_DIZITIZED)),  # 　面積の比率
            np.digitize(area_v, bins=self.bins(-100.0, 100.0, NUM_DIZITIZED)),
            np.digitize(
                center_pos_x, bins=self.bins(-50.0, 50.0, NUM_DIZITIZED)),
            np.digitize(
                center_pos_y, bins=self.bins(-50.0, 50.0, NUM_DIZITIZED)),
            np.digitize(
                center_v_x, bins=self.bins(-50.0, 50.0, NUM_DIZITIZED)),
...
class Environment:
    ...
    def reset(self):
        ...
        observation = (area_sum, area_v, center_pos_x,
                       center_pos_y, center_v_x, center_v_y)
    ...
    def act_env(self, observation, action):
        ...
        observation_next, _ = self.get_env(
            area_sum, center_pos_x, center_pos_y)
    ...
    def test(self, num_episodes):
        ...
        for episode in range(num_episodes):  # 試行数分繰り返す
            ...
            for step in range(MAX_STEPS):  # 1エピソードのループ
                ...
                if done:
                    reward = 1  # 目標を掴んだら報酬1を与える
                    images[0].save('success_episode.gif', save_all=True,
        ...
        # 最終エピソードの動き
        images[0].save('final_episode.gif', save_all=True,
